...
window.changeMainView = (viewId, element = null) => {
  ...
  const doNav = () => {
    ...
    if(viewId === 'view-exam-hub') { if(window.loadExamHub) window.loadExamHub(); }
    if(viewId === 'view-exam-hub') { if(window.loadExamHub) window.loadExamHub(); }
...
// ============================================================
// ONLINE EXAM MODULE (STUDENT)
// ============================================================
window.loadExamHub = async function() {
  const containerAvailable = document.getElementById('exam-available-list');
  const containerHistory = document.getElementById('exam-history-list');
  if(!containerAvailable || !containerHistory) return;
  
  if(!currentUser) {
    containerAvailable.innerHTML = '<p class="text-red-500 col-span-full">Vui lòng đăng nhập để xem đề thi!</p>';
    containerHistory.innerHTML = '<p class="text-red-500">Vui lòng đăng nhập để xem lịch sử!</p>';
    return;
  }
  
  containerAvailable.innerHTML = '<div class="loader mx-auto col-span-full"></div>';
  containerHistory.innerHTML = '<div class="loader mx-auto"></div>';
  ...
  try {
    ...
    if(activeExams.length === 0) {
      containerAvailable.innerHTML = '<p class="text-muted col-span-full">Hiện không có đề thi nào đang mở.</p>';
    } else {
      activeExams.forEach(exam => {
        containerAvailable.innerHTML += `
          <div class="glass-card bg-card p-5 rounded-2xl border border-theme flex flex-col gap-4 hover:border-orange-500/50 transition-colors">
            <div class="flex justify-between items-start">
              <span class="bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 px-2 py-1 rounded text-[10px] font-bold uppercase">${exam.subject || 'Chung'}</span>
              <span class="text-xs font-bold text-muted flex items-center gap-1"><i data-lucide="clock" class="w-3 h-3"></i> ${exam.duration} phút</span>
            </div>
            <h3 class="font-bold text-main line-clamp-2">${exam.title}</h3>
            <p class="text-xs text-muted">Số câu: ${exam.questionCount || 0}</p>
            <button class="mt-auto btn-primary w-full py-2 flex items-center justify-center gap-2 text-sm" onclick="window.startExam('${exam.id}')">
              <i data-lucide="play" class="w-4 h-4"></i> Bắt đầu làm bài
            </button>
          </div>
        `;
      });
    ...
    // Load history
    const historyQuery = query(collection(db, "exam_submissions"), where("userEmail", "==", currentUser.email));
    const histSnap = await getDocs(historyQuery);
    containerHistory.innerHTML = '';
    
    if(histSnap.empty) {
      containerHistory.innerHTML = '<p class="text-muted">Bạn chưa hoàn thành bài thi nào.</p>';
    } else {
      histSnap.forEach(d => {
        const sub = d.data();
        containerHistory.innerHTML += `
          <div class="glass-card bg-card p-4 rounded-xl border border-theme flex justify-between items-center hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
            <div>
              <h4 class="font-bold text-sm text-main line-clamp-1">${sub.examTitle}</h4>
              <p class="text-xs text-muted mt-1">Nộp lúc: ${new Date(sub.submittedAt).toLocaleString('vi-VN')}</p>
            </div>
            <div class="text-right shrink-0 ml-4">
              <p class="text-lg font-black text-orange-500">${sub.score}/${sub.maxScore || 10}</p>
              <p class="text-[10px] font-bold text-green-500">${sub.correctCount}/${sub.totalQuestions} câu</p>
            </div>
          </div>
        `;
      });
    ...
    containerAvailable.innerHTML = '<p class="text-red-500">Lỗi tải dữ liệu. Vui lòng thử lại.</p>';
  }
}

window.switchExamTab = function(tab) {
  const tabs = document.querySelectorAll('#exam-hub-tabs button');
  tabs.forEach(t => {
    t.classList.remove('text-primary', 'border-b-2', 'border-primary');
    t.classList.add('text-muted');
  });
  const activeTab = document.querySelector(`#exam-hub-tabs button[data-target="${tab}"]`);
  if(activeTab) {
  ...
  if(tab === 'available') {
    document.getElementById('exam-available-list').classList.remove('hidden');
    document.getElementById('exam-history-list').classList.add('hidden');
  } else {
    document.getElementById('exam-available-list').classList.add('hidden');
    document.getElementById('exam-history-list').classList.remove('hidden');
  }
}
...
window.startExam = async function(examId) {
  ...
  try {
    ...
    if(!docSnap.exists()) return showToast("Không tìm thấy đề thi!", true);
    currentExamData = docSnap.data();
    currentExamData.id = examId;
    
    // Check for draft
    const draftId = `${currentUser.email}_${examId}`;
    const draftSnap = await getDoc(doc(db, "exam_drafts", draftId));
    examDraftAnswers = {};
    if(draftSnap.exists()) {
      const draftData = draftSnap.data();
      examDraftAnswers = draftData.answers || {};
      examRemainingSeconds = draftData.remainingTime || (currentExamData.duration * 60);
    } else {
      examRemainingSeconds = currentExamData.duration * 60;
    }
    
    window.changeMainView('view-exam-take');
    
    // Setup UI
    document.getElementById('exam-take-title').textContent = currentExamData.title;
    document.getElementById('exam-take-subject').textContent = currentExamData.subject || '';
    document.getElementById('exam-pdf-viewer').src = currentExamData.pdfUrl || '';
    document.getElementById('exam-pdf-viewer').onload = () => {
      const loader = document.getElementById('exam-pdf-loading');
    ...
    console.error(e);
    showToast("Lỗi khi mở đề thi", true);
  }
}

window.leaveExam = function() {
  if(confirm("Bạn có chắc chắn muốn thoát? Dữ liệu đang làm sẽ được lưu nháp.")) {
    clearInterval(examTimerInterval);
    forceSaveDraft();
    window.changeMainView('view-exam-hub');
    document.getElementById('exam-pdf-viewer').src = '';
  }
}
...
function startExamTimer() {
  ...
  examTimerInterval = setInterval(() => {
    ...
    if(examRemainingSeconds <= 0) {
      clearInterval(examTimerInterval);
      showToast("Đã hết thời gian làm bài! Tự động nộp bài.", true);
...
function updateTimerUI() {
  ...
  if(!el) return;
  if(examRemainingSeconds <= 0) { el.textContent = "00:00:00"; return; }
  ...
  if(examRemainingSeconds < 300) { // 5 mins
    el.classList.replace('text-orange-500', 'text-red-500');
    el.classList.replace('bg-orange-50', 'bg-red-50');
    if(examRemainingSeconds % 2 === 0) el.classList.add('opacity-50');
    else el.classList.remove('opacity-50');
  }
}

function renderCompactAnswerForm() {
  const container = document.getElementById('exam-answer-form-container');
  container.innerHTML = '';
  
  if(!currentExamData.questions || currentExamData.questions.length === 0) {
    container.innerHTML = '<p class="text-muted">Đề thi chưa có cấu trúc đáp án.</p>';
    return;
  }
  
  currentExamData.questions.forEach((q, idx) => {
    const wrapper = document.createElement('div');
    wrapper.className = 'mb-6 p-4 bg-gray-50 dark:bg-slate-800/50 rounded-2xl border border-theme hover:border-orange-300 transition-colors duration-300';
    
    let qNumber = `Câu ${idx + 1}`;
    let inputHtml = '';
    
    if(q.type === 'multiple_choice') {
      const val = examDraftAnswers[q.id] || '';
      inputHtml = `
        <div class="flex gap-3 justify-center mt-3">
          ${['A', 'B', 'C', 'D'].map(opt => `
            <label class="relative cursor-pointer group">
              <input type="radio" name="q_${q.id}" value="${opt}" class="sr-only peer" ${val === opt ? 'checked' : ''} onchange="window.saveExamDraft('${q.id}', '${opt}')">
              <div class="w-10 h-10 rounded-full border-2 border-theme flex items-center justify-center font-bold text-sm text-muted peer-checked:border-orange-500 peer-checked:bg-orange-500 peer-checked:text-white transition-all group-hover:border-orange-300 shadow-sm">
                ${opt}
              </div>
            </label>
          `).join('')}
      ...
      const draftObj = examDraftAnswers[q.id] || {};
      inputHtml = `
        <div class="mt-3 space-y-2">
          ${['a', 'b', 'c', 'd'].map(opt => {
            const v = draftObj[opt] || '';
            return `
              <div class="flex items-center justify-between p-2 bg-white dark:bg-slate-800 rounded-xl border border-theme shadow-sm">
                <span class="font-bold text-main w-6 text-center">${opt}.</span>
                <div class="flex gap-2">
                  <label class="cursor-pointer group">
                    <input type="radio" name="q_${q.id}_${opt}" value="Đúng" class="sr-only peer" ${v === 'Đúng' ? 'checked' : ''} onchange="window.saveExamDraftTF('${q.id}', '${opt}', 'Đúng')">
                    <div class="px-5 py-1.5 rounded-lg border-2 border-transparent bg-gray-100 dark:bg-slate-700 text-xs font-bold text-muted peer-checked:bg-green-500 peer-checked:text-white transition-all group-hover:bg-gray-200 dark:group-hover:bg-slate-600">Đúng</div>
                  </label>
                  <label class="cursor-pointer group">
                    <input type="radio" name="q_${q.id}_${opt}" value="Sai" class="sr-only peer" ${v === 'Sai' ? 'checked' : ''} onchange="window.saveExamDraftTF('${q.id}', '${opt}', 'Sai')">
                    <div class="px-5 py-1.5 rounded-lg border-2 border-transparent bg-gray-100 dark:bg-slate-700 text-xs font-bold text-muted peer-checked:bg-red-500 peer-checked:text-white transition-all group-hover:bg-gray-200 dark:group-hover:bg-slate-600">Sai</div>
                  </label>
                </div>
              </div>
            `;
      ...
      inputHtml = `
        <div class="mt-3">
          <input type="text" class="input-glass w-full text-center font-bold !py-3 !rounded-xl" placeholder="Nhập đáp án của bạn..." value="${val}" oninput="window.saveExamDraft('${q.id}', this.value)">
        </div>
      `;
    }
    
    wrapper.innerHTML = `
      <div class="flex justify-between items-center mb-1 border-b border-theme pb-2">
        <span class="font-black text-main">${qNumber}</span>
        <span class="text-[10px] font-bold text-muted uppercase tracking-wider bg-white dark:bg-slate-900 px-2.5 py-1 rounded-md shadow-sm border border-theme">
...
window.saveExamDraft = function(qId, val) {
  examDraftAnswers[qId] = val;
  triggerAutoSave();
}
window.saveExamDraftTF = function(qId, optLabel, val) {
  if(!examDraftAnswers[qId]) examDraftAnswers[qId] = {};
  examDraftAnswers[qId][optLabel] = val;
  triggerAutoSave();
}

function triggerAutoSave() {
  const status = document.getElementById('exam-auto-save-status');
  if(status) {
    status.innerHTML = '<i data-lucide="refresh-cw" class="w-3 h-3 animate-spin"></i> Đang lưu...';
    status.classList.replace('text-green-500', 'text-yellow-500');
    lucide.createIcons();
  }
  
  clearTimeout(autoSaveTimeout);
  autoSaveTimeout = setTimeout(forceSaveDraft, 2000);
}

async function forceSaveDraft() {
  if(!currentExamData || !currentUser) return;
  const draftId = `${currentUser.email}_${currentExamData.id}`;
  try {
    await setDoc(doc(db, "exam_drafts", draftId), {
      examId: currentExamData.id,
      userEmail: currentUser.email,
      answers: examDraftAnswers,
      remainingTime: examRemainingSeconds,
      lastSavedAt: new Date().toISOString()
    });
    const status = document.getElementById('exam-auto-save-status');
    if(status) {
      status.innerHTML = '<i data-lucide="check-circle" class="w-3 h-3"></i> Đã lưu nháp';
...
window.submitExam = async function(isAutoSubmit = false) {
  ...
  if(btn) {
    btn.disabled = true;
    btn.innerHTML = '<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> ĐANG CHẤM ĐIỂM...';
    lucide.createIcons();
  }
  ...
  let correctCount = 0;
  const maxScore = 10;
  const totalPointsConfigured = currentExamData.questions.reduce((sum, q) => sum + (parseFloat(q.points) || 0), 0);
  const pointScale = totalPointsConfigured > 0 ? (maxScore / totalPointsConfigured) : 1;
  
  currentExamData.questions.forEach(q => {
    let qScore = 0;
    const userAns = examDraftAnswers[q.id];
    const qPoints = parseFloat(q.points) || 0;
    
    if(q.type === 'multiple_choice') {
      if(userAns === q.answer) { qScore = qPoints; correctCount++; }
    } 
    else if(q.type === 'true_false') {
      if(userAns && typeof userAns === 'object' && q.statements) {
        let correctStatements = 0;
        q.statements.forEach(stmt => {
          if(userAns[stmt.label] === stmt.answer) correctStatements++;
        });
        let ratio = 0;
        if(correctStatements === 1) ratio = 0.1;
        else if(correctStatements === 2) ratio = 0.25;
        else if(correctStatements === 3) ratio = 0.5;
        else if(correctStatements === 4) { ratio = 1.0; correctCount++; }
        
        qScore = qPoints * ratio;
      }
    }
    else if(q.type === 'short_answer') {
      if(userAns && q.answer) {
        const acceptableAns = q.answer.split('|').map(s => s.trim().toLowerCase());
        if(acceptableAns.includes(userAns.toString().trim().toLowerCase())) {
          qScore = qPoints;
          correctCount++;
        }
      }
    }
    score += qScore * pointScale;
  });
  
  const timeTaken = (currentExamData.duration * 60) - examRemainingSeconds;
  ...
  try {
    ...
    // Batch write
    const batch = writeBatch(db);
    batch.set(doc(collection(db, "exam_submissions")), submissionData);
    batch.delete(doc(db, "exam_drafts", draftId));
    await batch.commit();
    
    showToast("Nộp bài thành công!");
    window.showExamResult(submissionData);
    
  } catch(e) {
    console.error("Submit error", e);
    showToast("Lỗi nộp bài, vui lòng thử lại!", true);
    if(btn) {
      btn.disabled = false;
      btn.innerHTML = '<i data-lucide="send" class="w-5 h-5"></i> NỘP BÀI THI';
    }
  }
}

window.showExamResult = function(sub) {
  window.changeMainView('view-exam-result');
  document.getElementById('exam-pdf-viewer').src = ''; // Clean up iframe
  
  document.getElementById('result-exam-title').textContent = `Đề thi: ${sub.examTitle}`;
  document.getElementById('result-score').textContent = `${sub.score}/${sub.maxScore}`;
  document.getElementById('result-correct-count').textContent = `${sub.correctCount}/${sub.totalQuestions}`;
  
  const h = Math.floor(sub.timeTaken / 3600);
  const m = Math.floor((sub.timeTaken % 3600) / 60);
  const s = sub.timeTaken % 60;
  document.getElementById('result-time-taken').textContent = `${h > 0 ? h+':' : ''}${m.toString().padStart(2, '0')}:${s.toString().padStart(2, '0')}`;
  
  const pct = Math.round((sub.correctCount / sub.totalQuestions) * 100) || 0;
  document.getElementById('result-completion').textContent = `${pct}%`;
  
  if(currentExamData && currentExamData.allowReviewAfterSubmit) {
    const btnRev = document.getElementById('btn-review-exam');
    btnRev.classList.remove('hidden');
    btnRev.onclick = () => {
      document.getElementById('exam-review-section').classList.remove('hidden');
      renderExamReview(sub, currentExamData);
      btnRev.scrollIntoView({behavior: 'smooth'});
    };
  } else {
...
function renderExamReview(sub, exam) {
  const container = document.getElementById('exam-review-container');
  container.innerHTML = '';
  
  exam.questions.forEach((q, idx) => {
    const wrapper = document.createElement('div');
    wrapper.className = 'p-5 bg-white dark:bg-slate-800 rounded-2xl border border-theme shadow-sm';
    
    let html = `<p class="font-black text-main mb-3 border-b border-theme pb-2 flex items-center justify-between">
      <span>Câu ${idx + 1}</span>
      <span class="text-[10px] font-bold text-muted uppercase bg-gray-100 dark:bg-slate-900 px-2 py-0.5 rounded border border-theme shadow-sm">${q.type === 'multiple_choice' ? 'Trắc nghiệm' : q.type === 'true_false' ? 'Đúng/Sai' : 'Trả lời ngắn'}</span>
    </p>`;
    
    if(q.type === 'multiple_choice') {
      const uAns = sub.answers[q.id] || 'Chưa làm';
      const cAns = q.answer;
      html += `<div class="flex gap-3 mt-3">`;
      ['A', 'B', 'C', 'D'].forEach(opt => {
        let cls = 'border-theme text-muted';
        if(opt === cAns) cls = 'bg-blue-500 text-white border-blue-500 ring-2 ring-blue-500/30'; // correct answer
        else if(opt === uAns && uAns !== cAns) cls = 'bg-red-500 text-white border-red-500'; // wrong chosen
        else if(opt === uAns && uAns === cAns) cls = 'bg-green-500 text-white border-green-500 ring-2 ring-green-500/30'; // correct chosen
        
        html += `<div class="w-10 h-10 rounded-full border-2 flex items-center justify-center font-bold text-sm ${cls}">${opt}</div>`;
      });
      html += `</div>`;
    }
    else if(q.type === 'true_false') {
      ...
      q.statements.forEach(stmt => {
        const uv = uAns[stmt.label] || 'Chưa làm';
        const cv = stmt.answer;
        const isCorrect = uv === cv;
        html += `
          <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-700/50 rounded-xl border border-theme">
            <span class="font-black text-main w-6">${stmt.label}.</span>
            <div class="flex-1 flex justify-end gap-6 text-xs font-bold uppercase tracking-wider">
              <span class="${isCorrect ? 'text-green-500' : 'text-red-500'}">Bạn chọn: <span class="bg-white dark:bg-slate-800 px-2 py-1 rounded border border-theme shadow-sm ml-1">${uv}</span></span>
              <span class="text-blue-500">Đáp án: <span class="bg-white dark:bg-slate-800 px-2 py-1 rounded border border-theme shadow-sm ml-1">${cv}</span></span>
            </div>
          </div>
        `;
      });
      ...
      const uAns = sub.answers[q.id] || 'Chưa làm';
      const cAns = q.answer;
      const isCorrect = cAns.split('|').map(s=>s.trim().toLowerCase()).includes(uAns.trim().toLowerCase());
      html += `
        <div class="text-sm font-bold mt-4 space-y-2">
          <p class="${isCorrect ? 'text-green-500' : 'text-red-500'} flex items-center gap-2"><i data-lucide="${isCorrect ? 'check-circle' : 'x-circle'}" class="w-4 h-4"></i> Bạn điền: <span class="bg-gray-100 dark:bg-slate-700 px-3 py-1 rounded text-main">${uAns}</span></p>
          <p class="text-blue-500 flex items-center gap-2"><i data-lucide="info" class="w-4 h-4"></i> Đáp án đúng: <span class="bg-blue-50 dark:bg-blue-900/20 px-3 py-1 rounded border border-blue-200 dark:border-blue-800">${cAns.split('|').join(' HOẶC ')}</span></p>
        </div>
      `;
    }
