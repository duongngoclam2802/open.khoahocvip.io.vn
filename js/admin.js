...
// ==========================================
// 10. ONLINE EXAMS MANAGEMENT
// ==========================================
let examsList = [];
let editingExamId = null;
let examQuestions = [];

const SUPABASE_URL = "YOUR_SUPABASE_URL_HERE"; // User will replace this
const SUPABASE_ANON_KEY = "YOUR_SUPABASE_ANON_KEY_HERE"; // User will replace this
const SUPABASE_BUCKET = "exams";
...
async function loadExams() {
  ...
  try {
    ...
    if(snap.empty) {
      container.innerHTML = '<p class="text-muted col-span-full">Chưa có đề thi nào.</p>';
      return;
    }
    ...
    snap.forEach(d => {
      ...
      container.innerHTML += `
        <div class="glass-card bg-card p-5 rounded-2xl border border-theme flex flex-col gap-4">
          <div class="flex justify-between items-start">
            <span class="bg-orange-100 text-orange-600 px-2 py-1 rounded text-[10px] font-bold uppercase">${data.subject || 'Chung'}</span>
            <span class="${isPub ? 'text-green-500' : 'text-gray-500'} text-xs font-bold px-2 py-1 rounded-full border ${isPub ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50'}">${isPub ? 'Đang mở' : 'Bản nháp'}</span>
          </div>
          <h3 class="font-bold text-main line-clamp-2">${data.title}</h3>
          <p class="text-xs text-muted flex items-center gap-1"><i data-lucide="clock" class="w-3 h-3"></i> ${data.duration} phút | <i data-lucide="list" class="w-3 h-3 ml-2"></i> ${data.questions ? data.questions.length : 0} câu</p>
          <div class="flex gap-2 mt-auto pt-4 border-t border-theme">
            <button class="btn-secondary !py-1 flex-1 text-xs font-bold text-blue-500" onclick="editExam('${data.id}')">Sửa đề</button>
            <button class="btn-secondary !py-1 flex-1 text-xs font-bold text-green-500" onclick="viewExamSubmissions('${data.id}')">Kết quả</button>
            <button class="btn-secondary !py-1 !px-2 text-xs font-bold text-red-500" onclick="deleteDocHandler('exams', '${data.id}', loadExams)"><i data-lucide="trash-2" class="w-3 h-3"></i></button>
          </div>
        </div>
      `;
    });
    ...
    console.error(e);
    container.innerHTML = '<p class="text-red-500 col-span-full">Lỗi tải dữ liệu.</p>';
  }
}
...
document.getElementById('btn-add-exam')?.addEventListener('click', () => {
  editingExamId = null;
  document.getElementById('modal-exam-title').textContent = 'Thêm Đề Thi Mới';
  document.getElementById('exam-title').value = '';
  document.getElementById('exam-subject').value = 'Chung';
  ...
  examQuestions = [];
  renderExamQuestions();
  modalExam.classList.remove('hidden');
});

document.getElementById('btn-close-exam-modal')?.addEventListener('click', () => modalExam.classList.add('hidden'));
document.getElementById('btn-cancel-exam')?.addEventListener('click', () => modalExam.classList.add('hidden'));

window.editExam = function(id) {
  const exam = examsList.find(e => e.id === id);
  if(!exam) return;
  editingExamId = id;
  document.getElementById('modal-exam-title').textContent = 'Sửa Đề Thi';
  document.getElementById('exam-title').value = exam.title || '';
  document.getElementById('exam-subject').value = exam.subject || 'Chung';
  document.getElementById('exam-duration').value = exam.duration || '90';
  document.getElementById('exam-status').value = exam.status || 'draft';
  document.getElementById('exam-allow-review').checked = exam.allowReviewAfterSubmit !== false;
  document.getElementById('exam-pdf-url').value = exam.pdfUrl || '';
  examQuestions = exam.questions ? JSON.parse(JSON.stringify(exam.questions)) : []; // deep copy
  renderExamQuestions();
  modalExam.classList.remove('hidden');
};
...
document.getElementById('exam-pdf-upload')?.addEventListener('change', async (e) => {
  ...
  if(SUPABASE_URL === "YOUR_SUPABASE_URL_HERE") {
    return showToast("Vui lòng cấu hình SUPABASE_URL trong js/admin.js", true);
  }
  
  const progress = document.getElementById('exam-pdf-progress');
  ...
  try {
    ...
    document.getElementById('exam-pdf-url').value = publicUrl;
    showToast("Tải PDF thành công!");
  } catch(err) {
    console.error(err);
    showToast("Lỗi tải PDF: " + err.message, true);
  } finally {
...
document.getElementById('btn-add-q-mcq')?.addEventListener('click', () => {
  examQuestions.push({ id: generateId(), type: 'multiple_choice', answer: 'A', points: 0.2 });
  renderExamQuestions();
});
document.getElementById('btn-add-q-tf')?.addEventListener('click', () => {
  examQuestions.push({ 
    id: generateId(), type: 'true_false', points: 1.0,
    statements: [
      { label: 'a', answer: 'Đúng' },
      { label: 'b', answer: 'Đúng' },
      { label: 'c', answer: 'Sai' },
...
document.getElementById('btn-add-q-sa')?.addEventListener('click', () => {
  examQuestions.push({ id: generateId(), type: 'short_answer', answer: '', points: 0.5 });
  renderExamQuestions();
});
...
window.moveQuestion = function(idx, dir) {
  if(idx + dir < 0 || idx + dir >= examQuestions.length) return;
  const temp = examQuestions[idx];
  examQuestions[idx] = examQuestions[idx+dir];
  examQuestions[idx+dir] = temp;
  renderExamQuestions();
}

function renderExamQuestions() {
  const container = document.getElementById('exam-questions-container');
  const empty = document.getElementById('exam-empty-questions');
  container.innerHTML = '';
  ...
  examQuestions.forEach((q, idx) => {
    const div = document.createElement('div');
    div.className = 'p-4 bg-gray-50 dark:bg-slate-800/50 border border-theme rounded-xl relative group';
    
    let qControls = `
      <div class="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button class="p-1 hover:bg-white dark:hover:bg-slate-700 rounded text-muted" onclick="moveQuestion(${idx}, -1)"><i data-lucide="arrow-up" class="w-3 h-3"></i></button>
        <button class="p-1 hover:bg-white dark:hover:bg-slate-700 rounded text-muted" onclick="moveQuestion(${idx}, 1)"><i data-lucide="arrow-down" class="w-3 h-3"></i></button>
        <button class="p-1 hover:bg-red-100 rounded text-red-500" onclick="removeQuestion(${idx})"><i data-lucide="trash-2" class="w-3 h-3"></i></button>
      </div>
    `;
    ...
    if(q.type === 'multiple_choice') {
      innerHtml = `
        <div class="flex items-center justify-between mb-2">
          <span class="font-bold text-main text-sm">Câu ${idx + 1} <span class="text-[10px] bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded ml-2 uppercase">Trắc nghiệm</span></span>
          <div class="flex items-center gap-2 mr-20">
            <label class="text-xs text-muted">Điểm:</label>
            <input type="number" step="0.1" class="w-16 p-1 text-xs border border-theme rounded bg-white dark:bg-slate-900 text-center" value="${q.points}" onchange="examQuestions[${idx}].points = parseFloat(this.value)">
          </div>
        </div>
        <div class="flex gap-4">
          <label class="text-xs font-bold text-muted mt-1">Đáp án đúng:</label>
          <div class="flex gap-2">
            ${['A', 'B', 'C', 'D'].map(opt => `
              <label class="cursor-pointer flex items-center gap-1">
                <input type="radio" name="q_${q.id}" value="${opt}" ${q.answer === opt ? 'checked' : ''} onchange="examQuestions[${idx}].answer = this.value">
                <span class="text-sm font-bold text-main">${opt}</span>
              </label>
            `).join('')}
          </div>
        </div>
      ...
      innerHtml = `
        <div class="flex items-center justify-between mb-2">
          <span class="font-bold text-main text-sm">Câu ${idx + 1} <span class="text-[10px] bg-green-100 text-green-600 px-1.5 py-0.5 rounded ml-2 uppercase">Đúng/Sai</span></span>
          <div class="flex items-center gap-2 mr-20">
            <label class="text-xs text-muted">Điểm tối đa:</label>
            <input type="number" step="0.1" class="w-16 p-1 text-xs border border-theme rounded bg-white dark:bg-slate-900 text-center" value="${q.points}" onchange="examQuestions[${idx}].points = parseFloat(this.value)">
          </div>
        </div>
        <div class="grid grid-cols-2 gap-2 mt-2">
          ${q.statements.map((stmt, sIdx) => `
            <div class="flex items-center justify-between bg-white dark:bg-slate-900 p-2 border border-theme rounded-lg">
              <span class="font-bold text-main mr-2 w-4">${stmt.label}.</span>
              <select class="p-1 text-xs border border-theme rounded bg-gray-50 dark:bg-slate-800" onchange="examQuestions[${idx}].statements[${sIdx}].answer = this.value">
                <option value="Đúng" ${stmt.answer === 'Đúng' ? 'selected' : ''}>Đúng</option>
                <option value="Sai" ${stmt.answer === 'Sai' ? 'selected' : ''}>Sai</option>
              </select>
            </div>
          `).join('')}
        </div>
      ...
      innerHtml = `
        <div class="flex items-center justify-between mb-2">
          <span class="font-bold text-main text-sm">Câu ${idx + 1} <span class="text-[10px] bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded ml-2 uppercase">Trả lời ngắn</span></span>
          <div class="flex items-center gap-2 mr-20">
            <label class="text-xs text-muted">Điểm:</label>
            <input type="number" step="0.1" class="w-16 p-1 text-xs border border-theme rounded bg-white dark:bg-slate-900 text-center" value="${q.points}" onchange="examQuestions[${idx}].points = parseFloat(this.value)">
          </div>
        ...
        <div class="mt-2">
          <input type="text" class="w-full p-2 text-sm border border-theme rounded-lg bg-white dark:bg-slate-900" placeholder="Đáp án (ngăn cách bằng dấu | nếu có nhiều đáp án. VD: 1|Một)" value="${q.answer}" onchange="examQuestions[${idx}].answer = this.value">
        </div>
      `;
    }
    
    div.innerHTML = qControls + innerHtml;
    container.appendChild(div);
  });
  lucide.createIcons();
}
...
document.getElementById('btn-save-exam')?.addEventListener('click', async () => {
  ...
  const pdfUrl = document.getElementById('exam-pdf-url').value;
  
  if(!title || !duration || !pdfUrl) return showToast("Vui lòng điền đủ Tên, Thời gian và Link PDF!", true);
  if(examQuestions.length === 0) return showToast("Đề thi phải có ít nhất 1 câu hỏi!", true);
  
  const examData = {
    title, subject, 
    duration: parseInt(duration),
    status, allowReviewAfterSubmit: allowReview,
    pdfUrl,
  ...
  try {
    if(editingExamId) {
      ...
      showToast("Cập nhật đề thi thành công!");
    } else {
      examData.createdAt = new Date().toISOString();
      await addDoc(collection(db, "exams"), examData);
      showToast("Thêm đề thi mới thành công!");
    }
    modalExam.classList.add('hidden');
    loadExams();
  } catch(e) {
    console.error(e);
    showToast("Lỗi khi lưu đề thi!", true);
  } finally {
...
// View Submissions
const modalExamSubs = document.getElementById('modal-exam-subs');
document.getElementById('btn-close-exam-subs-modal')?.addEventListener('click', () => modalExamSubs.classList.add('hidden'));

window.viewExamSubmissions = async function(examId) {
  modalExamSubs.classList.remove('hidden');
  const tbody = document.getElementById('exam-subs-table-body');
  tbody.innerHTML = '<tr><td colspan="5" class="p-4 text-center"><div class="loader mx-auto"></div></td></tr>';
  
  try {
    const subsQuery = query(collection(db, "exam_submissions"), where("examId", "==", examId));
    const snap = await getDocs(subsQuery);
    
    if(snap.empty) {
      tbody.innerHTML = '<tr><td colspan="5" class="p-4 text-center text-muted">Chưa có ai nộp bài.</td></tr>';
      return;
    }
    ...
    subs.sort((a, b) => b.score - a.score || a.timeTaken - b.timeTaken);
    
    tbody.innerHTML = '';
    subs.forEach(s => {
      const h = Math.floor(s.timeTaken / 3600);
      const m = Math.floor((s.timeTaken % 3600) / 60);
      const sec = s.timeTaken % 60;
      const timeStr = `${h>0?h+'h ':''}${m}m ${sec}s`;
      
      tbody.innerHTML += `
        <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
          <td class="px-4 py-3 border-b border-theme font-bold text-main">${s.userEmail}</td>
          <td class="px-4 py-3 border-b border-theme font-black text-orange-500">${s.score}/${s.maxScore}</td>
          <td class="px-4 py-3 border-b border-theme text-green-500 font-bold">${s.correctCount}/${s.totalQuestions}</td>
          <td class="px-4 py-3 border-b border-theme text-muted text-xs">${timeStr}</td>
          <td class="px-4 py-3 border-b border-theme text-muted text-xs">${new Date(s.submittedAt).toLocaleString('vi-VN')}</td>
        </tr>
      `;
    });
  } catch(e) {
